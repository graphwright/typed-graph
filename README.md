# Typed-graph Sherlock: Domain Implementation

The formal definition of a typed graph appears in `docs/formal-defns.md`.
This repository is a **domain project** that builds on the typed-graph core
infrastructure (`tg-core`).

The 4-tuple $(T, \Phi, V, \tau)$ is realized as a small Pydantic class hierarchy
provided by the `tg-core` package: an `Instance` root (membership in $V$) with two
disjoint subclasses, `EntityInstance` and `BaseStatement`. `BaseStatement` is generic
over its subject and object types, so a predicate's domain and range are ordinary type
annotations checked by mypy and Pydantic.

**This repository provides**:
- A minimal example domain (`Person`, `Organization`, `WorksFor`) in `example.py`
- The **Sherlock Holmes canon** domain in `sherlock/` with ProbLog integration
- Demonstration of deterministic + probabilistic reasoning for mystery solving

**Core infrastructure** (`base`, `rules`, `datalog`, `serialize`, `graph`) is imported
from the `tg-core` dependency (`../tg-core`).
...
> The marker traits and `Inverse` / `get_inverse` below are implemented in the `base`
> module (from `tg-core`); the snippets show how a predicate type opts in. `Rule` is
> realized differently — as Python objects in the `rules` module evaluated by `datalog`,
> not as a mixin — see the last subsection.

### Marker traits

```python
class Trait: ...               # common base for all traits
class Symmetric(Trait): ...
class Transitive(Trait): ...
class Functional(Trait): ...          # each subject has at most one object
class InverseFunctional(Trait): ...   # each object has at most one subject
```

A predicate opts in simply by inheriting the mixin alongside the typed base. All traits
inherit from a common `Trait` base class for uniform introspection. Because traits are
ordinary base classes, they are introspectable at runtime with `issubclass`:
...
issubclass(Knows, Trait)              # True (common base)
